...
(OS-keyring); `sync_*` (state/pull/push/resolve_merge/configure_remote); and
`agent_*` (`agent_list_files` / `agent_read_file` / `agent_write_output` — the local
agent's repo reader + scoped writer, paths guarded by `fs_io::resolve_within`).
...
| A new dialogue **opening** (a typed, document-anchored occasion that becomes a conversation) | A `DialogueOpening` builder in `src/lib/dialogue-openings.ts` (pure: compose a deterministic context string + a fenced-`{deposit}` contract + a turn budget — an opening with no deposit is cut) reachable via `openDialogueOpening` on the ephemeral `state/dialogue-state.ts` slice; it renders through `features/tests-panel/OpeningDialogue.tsx` + `use-opening-dialogue.ts` (turn pips, soft-yield, the exit-chip slingshot) on the one `AIProvider.interlocutorTurn` method. Do NOT add a new AI method per opening — they share one contract (inquiry → converge → deposit). Assemble context from canonical records only (`lib/activity-brief.ts` etc.), never an AI-written digest. The entry affordance is always a deliberate `⊕`; the transcript is ephemeral. See [`docs/dialogue-design.md`](docs/dialogue-design.md). |
...
| A new AI provider | New `LLMClient` in `src/services/ai/clients/<name>-client.ts` — the ONE file importing that SDK. Wire its key + dispatch in `ai-provider-registry.ts`; seed it in `model-catalog.ts`. Components never import the SDK. (Gemini specifics: the catalog's ordered `GEMINI_CATALOG` is the single source — `model-defaults.ts` derives the default fallback ladder from it, and the per-minute throttle in `request-throttle.ts` reads each model's `requestsPerMinute` from it. Structured rate-limit parsing lives in `clients/gemini-error.ts`; `model-fallback.ts` stays provider-neutral, reading only the generic `__quotaScope`/`__retryDelayMs` hints.) |
| A new local-agent tool | A bound `AgentTool` in `src/services/ai/agent/tool-registry.ts` (closes over the `Repository` + `AIProvider`; gate filesystem-backed tools behind `enableFsTools` so the browser omits them). The loop, the prompted-JSON tool protocol, and the whole-text `buildAgentContext` live in `src/services/ai/agent/`; the surface is the single `AIProvider.runAgent` method + the `'runAgent'` call kind. A "routine" tool must delegate to an existing `AIProvider` method (its own call kind — never `'runAgent'`, or the loop recurses). This is the provider-agnostic local agent; it is SEPARATE from the Claude Agent SDK transport (`agent-sidecar/`) and must not touch it. |
