...
type GistPromptKey = 'gistAnalysisPrompt' | 'gistCompositionPrompt';

/** An editable engine prompt (project scope, sparse-diff save path, reset-to-default). */
function PromptField({ promptKey, label }: { promptKey: GistPromptKey; label: string }) {
 ...
 * composition exemplar is the single highest-leverage knob). Auto-saves; "Done" closes.
 */
      ...
      <div className="flex flex-col gap-5">
        ...
        <div>
          <Disclosure label="Analysis prompt">
            <PromptField promptKey="gistAnalysisPrompt" label="Stage A — analysis" />
          </Disclosure>
          <Disclosure label="Composition prompt">
            <PromptField promptKey="gistCompositionPrompt" label="Stage B — composition" />
          </Disclosure>
        </div>
