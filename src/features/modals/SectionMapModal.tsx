      ...
      return (
        <div 
          ...
          className={`
            ...
            ${isSelected ? 'bg-hld-cyan shadow-[0_0_12px_rgba(0,232,245,0.8)] z-20 scale-[1.3]' : 'bg-hld-muted-text-2 hover:bg-hld-text hover:scale-110 shadow-[0_0_4px_rgba(138,166,196,0.4)] z-10'}
          `}
          title={section.title || `Section ${section.id.split('-').pop()}`}
        >
          {isSelected && (
             <div className="absolute -top-[12px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[6px] border-l-transparent border-r-transparent border-t-hld-cyan animate-bounce cursor-default pointer-events-none"></div>
          )}
          
          <div className="opacity-0 group-hover:opacity-100 absolute top-[130%] left-1/2 -translate-x-1/2 whitespace-nowrap bg-hld-surface-3/95 px-2 py-1 rounded text-[10px] font-mono text-hld-text border border-hld-cyan/30 transition-opacity pointer-events-none z-50 shadow-xl backdrop-blur-sm">
            {section.title || `Sec`}
            {hasGoals && (
               <span className="block text-hld-muted-text-2 text-[8px] mt-0.5">{isSuccess ? 'Goals Met' : 'Goals Pending'}</span>
            )}
          </div>
        </div>
      );
    ...
    return (
      <div 
        ...
        className={`
          ...
          ${isSelected ? 'border-hld-cyan bg-hld-cyan/10 text-hld-text' : 'border-hld-border bg-hld-surface-2/90 hover:border-hld-cyan/50 text-hld-text'}
        `}
      >
        <div className="flex justify-between items-start mb-1">
          <h4 className={`font-bold text-sm line-clamp-2 ${isSelected ? 'text-hld-text' : 'text-hld-text'}`}>
            {section.title || `Section ${section.id.split('-').pop()}`}
          </h4>
          {hasGoals ? (
             isSuccess ? <CheckCircle2 size={14} className="text-hld-green shrink-0 ml-2" /> : <CheckCircle2 size={14} className="text-hld-yellow shrink-0 ml-2" />
          ) : (
             <Circle size={14} className="text-hld-border shrink-0 ml-2" />
          )}
        </div>
        <p className={`text-[10px] font-mono truncate ${isSelected ? 'text-hld-text/70' : 'text-hld-muted-text'}`}>
      ...
      <div className="flex flex-col items-center">
        <div className="shrink-0 relative z-10 w-fit">
          ...
          {section.children && section.children.length > 0 && (
            <div className="absolute top-[100%] left-1/2 w-[2px] h-[16px] bg-hld-magenta shadow-[0_0_6px_rgba(255,16,96,0.7)] -translate-x-1/2 z-0"></div>
          )}
        </div>
        {section.children && section.children.length > 0 && (
           <div className="flex items-start gap-4 relative mt-[16px] pt-[16px]">
              ...
              {section.children.length > 1 && (
                <div className="absolute top-0 left-[calc(14px)] right-[calc(14px)] h-[2px] bg-hld-magenta shadow-[0_0_6px_rgba(255,16,96,0.7)] z-0"></div>
              )}
              {section.children.map(child => (
                <div key={child.id} className="relative z-10 flex justify-center">
                   <div className="absolute top-[-16px] left-1/2 w-[2px] h-[16px] bg-hld-magenta shadow-[0_0_6px_rgba(255,16,96,0.7)] -translate-x-1/2 z-0"></div>
                   {renderTree(child)}
                </div>
              ))}
           </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div ref={modalRef} className="bg-hld-bg rounded-xl shadow-2xl w-full max-w-[95vw] h-[90vh] border border-hld-border flex overflow-hidden">
        ...
        {/* Main Interface */}
        <div className="flex-1 flex flex-col bg-hld-surface overflow-hidden">
          <div className="p-4 border-b border-hld-border bg-hld-surface flex justify-between items-center shrink-0">
             <div>
               ...
               <p className="text-[10px] uppercase tracking-widest text-hld-muted-text font-mono mt-1">Navigate & Plan Sections</p>
             </div>
             <div className="flex bg-hld-bg p-1 rounded-lg">
                <button 
                  onClick={() => setLayout('rows')}
                  className={`p-1.5 rounded flex items-center gap-2 text-xs font-mono uppercase tracking-widest transition-colors ${layout === 'rows' ? 'bg-hld-surface shadow-sm text-hld-cyan font-bold' : 'text-hld-muted-text hover:text-hld-text'}`}
                >
                  <AlignJustify size={14} /> Rows
                </button>
                <button 
                  onClick={() => setLayout('columns')}
                  className={`p-1.5 rounded flex items-center gap-2 text-xs font-mono uppercase tracking-widest transition-colors ${layout === 'columns' ? 'bg-hld-surface shadow-sm text-hld-cyan font-bold' : 'text-hld-muted-text hover:text-hld-text'}`}
                >
                  <LayoutTemplate size={14} /> Columns
                </button>
                <button 
                  onClick={() => setLayout('corkboard')}
                  className={`p-1.5 rounded flex items-center gap-2 text-xs font-mono uppercase tracking-widest transition-colors ${layout === 'corkboard' ? 'bg-hld-surface shadow-sm text-hld-cyan font-bold' : 'text-hld-muted-text hover:text-hld-text'}`}
                >
          ...
          <div className="flex-1 overflow-auto relative bg-hld-bg select-none" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.04) 1px, transparent 1px)', backgroundSize: '40px 40px', backgroundPosition: 'center center' }}>
             {/* Tech HUD Decals */}
             <div className="absolute top-4 left-4 w-12 h-12 border-t-2 border-l-2 border-hld-border pointer-events-none"></div>
             <div className="absolute top-4 right-4 w-12 h-12 border-t-2 border-r-2 border-hld-border pointer-events-none"></div>
             <div className="absolute bottom-4 left-4 w-12 h-12 border-b-2 border-l-2 border-hld-border pointer-events-none"></div>
             <div className="absolute bottom-4 right-4 w-12 h-12 border-b-2 border-r-2 border-hld-border pointer-events-none"></div>
                  ...
                  <div className="flex flex-col gap-16 h-full min-w-min w-max mx-auto items-center pt-12 pb-24">
                     {topLevelSections.map((l1, i) => (
                       <div key={l1.id} className="w-full flex flex-col items-center relative">
                          <div className="absolute -top-[32px] left-1/2 -translate-x-1/2 font-mono text-[10px] text-hld-cyan font-bold uppercase tracking-[0.3em] px-4 py-1 border border-hld-cyan/40 rounded bg-hld-cyan/10 shadow-[0_0_10px_rgba(0,232,245,0.2)] backdrop-blur-md z-30 pointer-events-none">
                            Chapter {i+1}
                          </div>
                          {renderTree(l1)}
                       </div>
                 ...
                 {topLevelSections.map((l1, i) => (
                   <React.Fragment key={l1.id}>
                     ...
                     <div className="w-[300px] shrink-0 flex flex-col">
                        <div className="font-mono text-[10px] text-hld-cyan font-bold uppercase tracking-[0.2em] mb-4 pb-2 border-b border-hld-border">
                          H1 Node / Sector {i+1}
                        </div>
                     ...
                     {l1.children && l1.children.length > 0 && (
                        <div className="w-[300px] shrink-0 flex flex-col px-4 border-l-2 border-dashed border-hld-border">
                           <div className="font-mono text-[10px] text-hld-cyan font-bold uppercase tracking-[0.2em] mb-4 pb-2 border-b border-hld-border">
                             ...
                             {l1.children.map(child => (
                               <div key={child.id} className="relative">
                                  ...
                                  {child.children && child.children.length > 0 && (
                                     <div className="pl-4 ml-2 border-l-2 border-hld-border mt-2 mb-4">
            ...
            {layout === 'corkboard' && (
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {topLevelSections.map(l1 => (
                    <div key={l1.id} className="bg-hld-surface-2/80 backdrop-blur-sm p-4 rounded-xl border border-hld-border shadow-inner">
                       <div className="mb-4">
                         {renderSectionCard(l1)}
                       </div>
                       <div className="space-y-2">
                         {l1.children && l1.children.map(child => (
                            <div key={child.id} className="pl-4 relative">
                              <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-hld-magenta/30" />
                              <div className="absolute left-0 top-6 w-3 h-[2px] bg-hld-magenta/30" />
                              {renderSectionCard(child)}
                              {child.children && child.children.map(grandchild => (
                                 <div key={grandchild.id} className="pl-4 relative mt-2">
                                    <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-hld-cyan/30" />
                                    <div className="absolute left-0 top-6 w-3 h-[2px] bg-hld-cyan/30" />
        ...
        <div style={{ width: panelWidth }} className="flex flex-col bg-hld-surface shrink-0 relative">
          <div className="p-4 border-b border-hld-border flex justify-between items-center bg-hld-surface/50">
            <h3 className="font-bold text-sm font-sans text-hld-text uppercase tracking-widest">Goal Editor</h3>
            <button 
              onClick={onClose}
              className="p-1.5 rounded hover:bg-hld-border text-hld-muted-text transition-colors"
            >
              <X size={16} />
            </button>
          </div>
