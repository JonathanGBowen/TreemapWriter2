...
export const CoachModal: React.FC<CoachModalProps> = ({
  ...
  const realizations = useStore(s => s.realizations);
  ...
  const currentInputHash = computeHash(`${markdown.length}|${JSON.stringify(structureData)}|${JSON.stringify(structuralParts)}|${JSON.stringify(structuralEdges)}|${JSON.stringify(realizations)}`);
        ...
        <div className="p-3 border-t flex justify-end border-hld-border bg-hld-surface shrink-0">
          ...
          {actionPlan && (
            <button
               ...
               className="px-4 py-2 bg-transparent border border-hld-border text-hld-text text-[11px] font-mono uppercase tracking-[0.1em] hover:bg-hld-surface-2 transition-colors"
            >
              Reset
            </button>
          )}
